...
ImagePPM::ImagePPM( const unsigned int _width, const unsigned int _height,
        const Rgb &c ) : width( _width ), height( _height )
{
    ...
    try
    {
        ...
        *is >> w >> h >> b;
        ...
        for ( unsigned int i = 0; i < w * h; ++i )
        {
            ...
            this->pixels[ i ].r = std::complex<double>(pixel[ 0 ] / 255.0, 0.0);
            this->pixels[ i ].g = std::complex<double>(pixel[ 1 ] / 255.0, 0.0);
            this->pixels[ i ].b = std::complex<double>(pixel[ 2 ] / 255.0, 0.0);
        }
    }
...
void ImagePPM::print() const 
{
    ...
    try
    {
        ...
        *os << this->width << " " << this->height << "\n255\n";
        ...
        {
            r = static_cast<unsigned char>( std::min( 1.0, 
                        this->pixels[ i ].r.real() ) * 255 );
            g = static_cast<unsigned char>( std::min( 1.0, 
                        this->pixels[ i ].g.real() ) * 255 );
            b = static_cast<unsigned char>( std::min( 1.0, 
                        this->pixels[ i ].b.real() ) * 255 );
            *os << r << g << b;
        }
    }
    catch ( const char* err )
